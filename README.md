...
A local-first personal CRM for the terminal. Store contacts, organizations, interactions, deals, and tasks in a local SQLite database. Every command outputs structured data (table, JSON, CSV) making it equally usable by humans and AI agents. Ships with a built-in MCP server so Claude and other AI agents can search, query, and update your CRM directly.
...
**People** — contacts with name, email, phone, title, location, social links, notes, an AI-maintained `summary` field (living dossier of key facts about the person), plus custom fields
...
AI calls:
1. crm_interaction_log(type="meeting", person_id=42, subject="Timeline agreement")
2. crm_deal_update(id=10, stage="negotiation")
3. crm_task_create(title="Follow up on signed contract", person_id=42, due="next week")
4. crm_person_update_summary(person_id=42, summary="CTO at Acme Corp. Working on $15K website redesign deal. Agreed on timeline Mar 2026, expecting signed contract by mid-March. Prefers async communication. Met at ReactConf 2025. Colleague of Tom Baker.")
```
...
crm-cli/
  src/
    cli/
      ...
      commands/
        person.ts, org.ts, interaction.ts, tag.ts,
        deal.ts, task.ts, search.ts, context.ts,
        import.ts, export.ts, config.ts, stats.ts, mcp.ts
    db/
      index.ts                  # connection, migration runner
      migrations/
        001_initial.sql         # complete schema
      repositories/
        person.repo.ts, org.repo.ts, interaction.repo.ts,
        tag.repo.ts, deal.repo.ts, task.repo.ts
    mcp/
      server.ts                 # MCP server setup
      tools.ts                  # tool definitions
      resources.ts              # resource definitions
    formatters/
      table.ts, json.ts, csv.ts, context.ts
    models/
      types.ts                  # Zod schemas + TS types
    utils/
      dates.ts, config.ts, editor.ts
  tests/
    unit/
    integration/
    fixtures/
...
**Phase 1 — Foundation (MVP):** Database schema + migrations, Person CRUD, Organization CRUD, basic interaction logging (`crm log`), full-text search, table + JSON output, shell completions
...
**Phase 3 — AI Integration:** MCP server with full tool suite, MCP resources, CLAUDE.md, natural language date parsing

**Phase 4 — Productivity:** Deals/pipeline, tasks/follow-ups, `crm status` dashboard, auto-backups

**Phase 5 — Polish:** fzf integration, duplicate detection/merge, bulk operations, man page
...
5. **Zero friction** — `npx crm-cli person add "Jane Smith"` and you're running
